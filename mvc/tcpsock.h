...
public:
	static bool Init();
	static bool UnInit();
	static RemotePeer CreateClient(const std::string &ip, unsigned short port) throw(std::exception);	
	static LocalPeer CreateServer(const std::string &ip, unsigned short port) throw(std::exception);	
};
